...
export default function CartPage() {
    ...
    const fetchCartItems = async () => {
      try {
        ...
        if(isFlag){
            ...
            setCartId(cartId);
            const transformedCartItems = cartData.cartItemList.map((item: any) => ({
              id: item.id,
              type: item.type,
              name: item.name,
  ...
  const handleDeleteConfirmation = () => {
    // 체크된 아이템 ID 배열 생성
    const idsToDelete = cartItems.filter((item) => item.isChecked).map((item) => item.id);
  ...
  const handleReserve = () => {
    ...
    const reservedItemsString = encodeURIComponent(JSON.stringify(reservedItems));
  ...
  const homeIconUrl = './assets/icon/ic_market_home.png';
  ...
  return (
    <div className="flex flex-col items-center w-full bg-gray-100 py-4">
      <div className="w-[360px] bg-white">
        <Header url1={url} title={title} imageUrl={homeIconUrl} url2={homeUrl} />
        <TopBar
        ...
        <div className="p-4 border-t-2 border-gray-300">
          <h2 className="text-sm font-semibold text-gray-900 mb-2">할인 및 결제 정보</h2>
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex justify-between items-center mb-4">
            <span className="text-sm text-gray-600">총 {cartItems.length}건</span>
